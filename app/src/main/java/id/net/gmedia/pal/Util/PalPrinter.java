    ...
    public void printPelunasan(PelunasanModel pelunasan){
        ...
        try {
            ...
            for(int i = 0; i < listItem.size(); i++){
                ...
                String nama = t.getNota();
                String tglNota = t.getTglNota();
                ...
                if(tglNota.length() > TGL_MAX){
                    n = Math.max((int)Math.ceil((double)tglNota.length()/(double)TGL_MAX), n);
                }
                ...
                String[] nama_array = leftAligned(nama, NAMA_MAX, n);
                String[] tgl_array = leftAligned(tglNota, TGL_MAX, n);
                String[] harga_total_array = rightAligned(dibayar, HARGA_TOTAL_MAX, n);

                for(int j = 0; j < n; j++){
                    getOutputStream().write(String.format(Locale.getDefault(), "%s %s %s\n",
                            nama_array[j], tgl_array[j], harga_total_array[j]).getBytes());
                }
